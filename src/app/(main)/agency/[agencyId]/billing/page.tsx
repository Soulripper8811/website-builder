...
const Billing = async ({ params }: Props) => {
  ...
  //khuch error aaya toh idear aana
  const charges = await stripe.charges.list({
    limit: 50,
    customer: agencySubscription?.Subscription?.customerId,
  });
      ...
      <div className="flex flex-col lg:!flex-row justify-between gap-8">
        ...
        {addOns.data.map((addOn) => (
          <PricingCard
            ...
            highlightDescription="Get priority support and skip the long wait with a click of a button."
          />
        ))}
      </div>
